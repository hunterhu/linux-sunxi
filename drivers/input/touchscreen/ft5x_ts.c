...
void ft5x_delay_ms(FTS_WORD  w_ms)
{
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(u8* pbt_buf, u16 dw_lenth)
{
    ...
    for (j=0;j<packet_number;j++)
    {
        ...
        ft5x_delay_ms(FTS_PACKET_LENGTH/6 + 1);
...
int fts_ctpm_auto_clb(void)
{
    ...
    ft5x_delay_ms(100);                       //make sure already enter factory mode
...
//	pr_info("==INIT_WORK=\n");
	...
	if (err < 0) {
		dev_err(&client->dev, "ft5x_ts_probe: request irq failed:err code:%i\n", err);
