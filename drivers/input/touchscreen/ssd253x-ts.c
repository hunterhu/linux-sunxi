...
#define SCREEN_MAX_HEIGHT		(screen_max_y)
#define SCREEN_MAX_WIDTH		(screen_max_x)
...
static unsigned long int touch_size_x = 960;
static unsigned long int touch_size_y = 640;
...
static int ctp_fetch_sysconfig_para(void)
{
	...
	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp_para", "ctp2_used", &ctp_used, 1)){
	...
	if(SCRIPT_PARSER_OK != script_parser_fetch_ex("ctp_para", "ctp2_name", (int *)(&name), &type, sizeof(name)/sizeof(int))){
		pr_err("%s: script_parser_fetch err. \n", __func__);
		goto script_parser_fetch_err;
	}
	...
	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp_para", "ctp2_twi_addr", &twi_addr, sizeof(twi_addr)/sizeof(__u32))){
		pr_err("%s: script_parser_fetch err. \n", name);
		goto script_parser_fetch_err;
	}
